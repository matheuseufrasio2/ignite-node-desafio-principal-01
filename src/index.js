...
app.put('/todos/:id', checksExistsUserAccount, (request, response) => {
  ...
  if (!todo) {
    return response.status(404).json({ error: "This todo does not exists to this user." });
  }

  const indexOfTodo = user.todos.indexOf(todo, 0);
  user.todos[indexOfTodo].title = title;
  user.todos[indexOfTodo].deadline = new Date(deadline);

  return response.status(200).json(user.todos[indexOfTodo]);
});
